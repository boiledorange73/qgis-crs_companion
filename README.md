...
CRS data and most plugin UI strings are stored in `crs_data/crs_companion.json`.
...
Most visible strings are externalized in the `ui` section of `crs_data/crs_companion.json`.
Translation source files are also included in `i18n/` for future Qt-based UI strings.
Compile `.ts` files to `.qm` with Qt Linguist tools in your QGIS/Qt environment if needed.

## Install

Zip the `crs_companion` folder, then install it from QGIS Plugin Manager using "Install from ZIP".
